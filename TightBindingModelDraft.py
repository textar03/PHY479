...
class TightBindingModel:
    ...
    def H_k(self, k_val):
        ...
        for (i, j, R), T_ij in self.hopping_element.items():
            R_cartesian = self.lattice_vec.T @ np.array(R)
            delta_vec = self.pos_orbitals[j] - self.pos_orbitals[i]
            geo_factor = np.exp(1j * np.dot(k_momentum, R_cartesian + delta_vec))
...
'''
    ...
    def H_ij(self, nsites):
        Htot = np.zeros((nsites*self.norb, nsites*self.norb), dtype=complex)
        
        #creating an index function to keep track of the cell, orbital information
        return 0
'''
